...
const BRAIN_DIR = process.env.ANTIGRAVITY_BRAIN_DIR || path.join(os.homedir(), '.gemini', 'antigravity', 'brain');
...
app.post('/api/orchestrator/command', (req, res) => {
  try {
    const { message, sender = 'Operador' } = req.body;
...
function startServer(port) {
  const server = app.listen(port, () => {
    console.log(`\n=============================================================`);
    console.log(`🏢 ANTIGRAVITY COCKPIT 2D PIXEL ART — SERVER RUNNING`);
    console.log(`📡 URL Principal: http://localhost:${port}`);
    console.log(`📊 SSE Stream:    http://localhost:${port}/api/stream`);
    console.log(`🛡️ Antigravity Foundry: Governança Integrada Dual-Layer`);
